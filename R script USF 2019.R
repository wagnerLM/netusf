#####  Curso #####
# Exemplo: DASS21, instrumento para avaliação de afetos negativos/sintomas transdiagnósticos
# Disponível em http://www2.psy.unsw.edu.au/dass/Portuguese/DASS%2021%20Brazilian%20Portuguese%20Tucci.pdf
...
### Exemplo paramétrico (Gaussian network)
...
# grupos por dimensão (estresse, ansiedade, depressão)
# estresse c(1,6,8,11,12,14,18)
# ansiedade c(2,4,7,9,15,19,20)
# depressão c(3,5,10,13,16,17,21)
...
dass_fa
# EFA tem algumas limitações (http://eiko-fried.com/wp-content/uploads/Fried_2016-04481-001.pdf)

# Correlações
cor.plot(cor_auto(dasspoly_sub),numbers = T)

# Correlações em redes (http://psychosystems.org/files/Literature/EpskampEtAl2012.pdf)
# Rede correlações bivariadas
...
# Rede de correlações parciais (http://psycnet.apa.org/record/2018-13501-001?doi=1)
...
# Visualização de vértices específicos
flow(dasspoly_sub_g, "DASS10")

# Matriz esparsa de correlação parcial regularizada ou matriz de predição
View(getWmat(dasspoly_sub_g))
cor.plot(getWmat(dasspoly_sub_g),numbers = T)

# Modelo com todos os itens
# definindo grupos
dassgroups<- list("Estresse"=c(1,6,8,11,12,14,18),"Ansiedade"=c(2,4,7,9,15,19,20),"Depressão"=c(3,5,10,13,16,17,21))
dassgroups

# estimando a rede de correlações parciais
...
# há outras opções como walktrap ou spinglass
# há tentativas de igualar o conceito de comunidade e 'variável latente', contudo há críticas a esta abordagem
